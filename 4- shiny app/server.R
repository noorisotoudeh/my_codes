...
scBubbHeat <- function(inpConf, inpMeta, inp, inpGrp, inpPlt, 
  ...
  if(inpGrp=="Clusters"){
    ...
    for (i in seq_along(1:nTreatment)){
      ...
      n1[[i]] = nrow(req_meta_data[which(req_meta_data$treatment==Treatment[[i]]),])
    ...
    out <- mapply(function(x,y) DataFrame(x,sampleID=rep(req_meta_data[which(req_meta_data$treatment==y),]$V1,1,each=n2)), out,Treatment)


    out <- lapply(out, function(x) DataFrame(x,grpBy=rep("treatment",nrow(x))))
...
output$sc1c2oup.png <- downloadHandler( 
  ...
  scvolcano_m <- function(inpMarkers_m){
    ...
    ggData$Significance = ifelse(ggData$p_val_adj < minfdr,
                                 # & abs(ggData$avg_log2FC) >= 0.58,
                                 ifelse(ggData$avg_log2FC > 0.0
                                        ,sc2def$treatment1,sc2def$treatment2)
                                 ,'Not siginficant'
    )
    ggData$Significance <- factor(ggData$Significance
                                  , levels = c(sc2def$treatment1,sc2def$treatment2,'Not siginficant'))
...
output$sc2d1oupTxt <- renderUI({ 
  x <- input$sc2d1grp
  # # updateTextAreaInput(session, "sc1d1inp", value = paste0(sc1def[[x]], collapse = ", "))
  textAreaInput("sc2d1inp", HTML("List of gene names (Max 50 genes, separated by , or ; or newline):"),
                height = "110px",width = "600px",
...
output$sc2d1oup.pdf <- downloadHandler(
  ...
  content = function(file) { ggsave( 
    ...
    plot = scBubbHeat2(sc2conf, sc2meta, input$sc2d1inp, input$sc2d1grp, input$sc2d1plt, 
                      input$sc2d1sub1, input$sc2d1sub2, "sc2gexpr.h5", sc2gene, 
                      input$sc2d1scl, input$sc2d1row, input$sc2d1col, 
